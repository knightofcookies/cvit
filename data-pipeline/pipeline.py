...
SARVAM_PAGE_LIMIT = (
    450  # Conservative safety buffer (API limit is 500, but 480 causes failures)
)
SARVAM_SIZE_LIMIT_BYTES = 190 * 1024 * 1024  # 190MB Safety buffer (Limit is 200MB)
                    ...
                    for result in search:
                        ...
                        if existing:
                            ...
                            new_item = Manifest(
                                ...
                                sarvam_lang_code=get_language_code(raw_lang),
                                status="queued",
                            )
        ...
        with self.session_scope() as session:
            tasks = (
                ...
                .filter(
                    ...
                    Manifest.sarvam_lang_code.isnot(None),
                )
                .limit(limit)
                .all()
            )
...
if __name__ == "__main__":
    ...
    try:
        while True:
            # Run Downloader
            pipeline.run_downloader(limit=5)

            # Run Submitter
            pipeline.run_ocr_submitter(limit=10)

            # Run Poller
            pipeline.run_ocr_poller()
